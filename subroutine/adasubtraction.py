...
from pylops.utils.backend import get_toeplitz
...
def convmtx1(h, n):
    ...
    if len(h) < n:
        ...
        col_1 = ncp.r_[h, ncp.zeros(n , dtype=h.dtype)]
...
def adasubtraction(data, multiples, nfilt, solver, solver_dict, nwin=(30,150),  clipping=False):
    ...
    filts_est = np.zeros((num_patches, nfilt))
    ...
    for i in range(num_patches):
        ...
        for j in range(nwin[0]):
            C = convmtx1(multiple_patch_i[j], nfilt)
        ...
        no_multiples = multiple_patch_i < 0.0001
        if no_multiples.all():
            filt_est = np.ones(nfilt)
        else:
            # solve for the filter
            if solver=='lsqr':
                filt_est = lsqr(CopStack, dataStack, x0=np.asarray(solver_dict['x0']), niter=solver_dict['niter'], damp=solver_dict['damp'])[0]
            elif solver=='ADMM':
